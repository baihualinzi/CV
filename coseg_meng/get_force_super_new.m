function phi1=get_force_super_new(img1,idx,phi,alpha1,alpha2,alpha3,alpha4,alpha5,alpha7,nn,super,large_setp)
img{1,1}=double(super(nn).color(:,1));
img{1,2}=double(super(nn).color(:,2));
img{1,3}=double(super(nn).color(:,3));
% idx1=idx{nn};
idx1=find(super(nn).vec==-1);
% vpts1=vpts{nn};
vpts1=find(super(nn).vec==0);
%% exterior region
dkd=sum(super(nn).num(vpts1));
E2=[];
for i=1:length(idx1)
        ...
        E2(i)=sum(super(nn).num(k3))/dkd;
end
% E2=E2';
BB=zeros(1,length(super(nn).vec));
BB(idx1)=E2;
%%
E2=BB(super(nn).label(idx{nn}))';
...
%     E2=[];
    ...
    img{2,1}=double(super(ki).color(:,1));
    img{2,2}=double(super(ki).color(:,2));
    img{2,3}=double(super(ki).color(:,3));
    upts2=find(super(ki).vec~=0);
    dkd=sum(super(ki).num(upts2));
    if isempty(upts2)
    ...
    for i=1:length(idx1)
        ...
        E1(i)=sum(super(ki).num(k3))/dkd;
    end
    end
    BB=zeros(1,length(super(nn).vec));
    BB(idx1)=E1;
%%
    E1=BB(super(nn).label(idx{nn}))';    
    E=[E,E1];
    kk=kk+1;
...
if  isempty(vpts1)
        ...
        F= E*beta+alpha4*E2+E4*beta1;
%%
        F=F./max(abs(F));%normalization
        F1=F;
end
%%
    curvature = get_curvature(phi1,idx{nn});  %  curvature penalty
    nei=zeros(length(F1),1);
    nei=alpha5+nei;
    dphidt = F1 + alpha1*curvature + nei;  % gradient descent to minimize energy
    %-- maintain the CFL condition
    if large_setp==1
        an_meng=max(min([size(phi1,1),size(phi1,2)])/30,1);
    else
        an_meng=1;
    end
    dt = an_meng/(max(dphidt)+eps);
    %-- evolve the curve
    phi1(idx{nn}) = phi1(idx{nn}) + dt.*dphidt;
    %-- Keep SDF smooth
    phi1 = sussman(phi1, .5);
