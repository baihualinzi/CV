...
function phi = co_segmentation_new(img,mm,nn,num_iter,parameter)
%%
...
phi=[];
for i=1:length(mm)
    phi{i} = mask2phi(mm{i});%零水平集函数
end

for its = 1:num_iter   % Note: no automatic convergence test
 ...
 %% evolution
    for i=1:length(phi)
        ...
        %% the first images
        phi{i}=get_force_new(img,idx,upts,vpts,phi,alpha1,alpha2,alpha3,alpha4,alpha5,alpha7,i);
    end
